...
class Lapan_api_library
{
	...
	private $api_endpoint_gateway = URL_API_GATEWAY;
	...
	public function call_gateway($func, $args=array(), $timeout = 300)
	{
		return $this->makeRequest_gateway($func, $args, 0);
	}
	/**
	...
	private function makeRequest_gateway($func, $args=array(), $timeout = 300)
	{      
		$url = $this->api_endpoint_gateway.'/'.$func;
		if (function_exists('curl_init') && function_exists('curl_setopt')){
		    $ch = curl_init();
		    curl_setopt($ch, CURLOPT_URL, $url);
		    curl_setopt($ch, CURLOPT_USERAGENT, 'PHP-MCAPI/2.0');       
